...
def layer_norm_ref(
    ...
    if upcast:
        ...
        x1 = x1.cast("float32") if x1 is not None else None
        weight1 = weight1.cast("float32") if weight1 is not None else None
        bias1 = bias1.cast("float32") if bias1 is not None else None
    if x1 is not None:
    ...
    out = F.layer_norm(x.cast(weight.dtype), x.shape[-1:], weight=weight, bias=bias, epsilon=eps).cast(dtype)
    if weight1 is None:
        return out if not prenorm else (out, x)
    else:
        out1 = F.layer_norm(x.cast(weight1.dtype), x.shape[-1:], weight=weight1, bias=bias1, epsilon=eps).cast(dtype)
...
def rms_norm_ref(
    ...
    out = ((x * rstd * weight) + bias if bias is not None else (x * rstd * weight)).cast(dtype)
    if weight1 is None:
        return out if not prenorm else (out, x)
    else:
        out1 = ((x * rstd * weight1) + bias1 if bias1 is not None else (x * rstd * weight1)).cast(dtype)
        return (out, out1) if not prenorm else (out, out1, x)
...
def config_prune(configs):
    ...
    #         device_name = paddle.device.cuda.get_device_properties(0).name
...
def _layer_norm_fwd(
    ...
    _layer_norm_fwd_1pass_kernel[(M,)](
        ...
        x1.strides[0] if x1 is not None else 0,
        y1.strides[0] if y1 is not None else 0,
        M,
        N,
        eps,
        dropout_p,
...
def _layer_norm_bwd(
    ...
    if dresidual is not None:
        ...
        assert tuple(dresidual.shape) == (M, N)
    assert weight.shape[0] == N
    assert weight.strides[-1] == 1
    if bias is not None:
        assert bias.strides[-1] == 1
    ...
    if seeds is not None:
        ...
        assert seeds.shape[0] == M if not has_x1 else M * 2
    if rowscale is not None:
        assert rowscale.is_contiguous()
        assert rowscale.shape[0] == M
    # allocate output
    dx = paddle.empty_like(x) if x_dtype is None else paddle.empty([M, N], dtype=x_dtype)
    dresidual_in = (
        paddle.empty_like(x)
        if has_residual and (dx.dtype != x.dtype or dropout_p > 0.0 or rowscale is not None or has_x1)
        else None
    )
    dx1 = paddle.empty_like(dx) if (has_x1 and dropout_p > 0.0) else None
    ...
    _db = paddle.empty((sm_count, N), dtype=paddle.float32) if bias is not None else None
    _dw1 = paddle.empty_like(_dw) if weight1 is not None else None
    _db1 = paddle.empty_like(_db) if bias1 is not None else None
    ...
    _layer_norm_bwd_kernel[grid](
        ...
        dy1.strides[0] if dy1 is not None else 0,
        dx1.strides[0] if dx1 is not None else 0,
    ...
    dw1 = _dw1.sum(0).cast(weight1.dtype) if weight1 is not None else None
    db1 = _db1.sum(0).cast(bias1.dtype) if bias1 is not None else None
...
class LayerNormFn(paddle.autograd.PyLayer):
    ...
    def forward(
        ...
        if rowscale is not None:
            rowscale = rowscale.reshape(
                [
                    -1,
                ]
            ).contiguous()
        residual_dtype = residual.dtype if residual is not None else (paddle.float32 if residual_in_fp32 else None)
    ...
    def backward(ctx, dy, *args):
        x, weight, bias, weight1, bias1, rowscale, seeds, mean, rstd = ctx.saved_tensor()
...
class RMSNorm(nn.Layer):
    def __init__(self, hidden_size, eps=1e-5, dropout_p=0.0, epsilon=None, dtype=None):
        super().__init__()
...
class LayerNormLinearFn(paddle.autograd.PyLayer):
    @staticmethod
    @custom_fwd
    def forward(
        ctx,
        ...
        y, _, mean, rstd, residual_out, *rest = _layer_norm_fwd(
            ...
            eps,
            residual,
            out_dtype=None if is_autocast_enabled() else get_autocast_gpu_dtype(),
            residual_dtype=residual_dtype,
            is_rms_norm=is_rms_norm,
        )
        y = y.reshape(x_shape_og)
        dtype = get_autocast_gpu_dtype() if is_autocast_enabled() else y.dtype
    ...
    def backward(ctx, dout, *args):
        ...
        dy = F.linear(dout, linear_weight, transpose_y=True)
