...
def _chunk_scan_fwd_kernel(
    ...
    C_ptr += (
        pid_b * stride_C_batch + pid_c * chunk_size * stride_C_seqlen + (pid_h // nheads_ngroups_ratio) * stride_C_head
    )
    ...
    if IS_TRITON_22 or pid_c > -1:
        ...
        if BLOCK_SIZE_DSTATE <= 128:
            C = tl.load(
                C_ptrs, mask=(offs_m[:, None] < chunk_size_limit) & (offs_k_dstate[None, :] < dstate), other=0.0
            )
            prev_states = tl.load(
                prev_states_ptrs, mask=(offs_k_dstate[:, None] < dstate) & (offs_n[None, :] < hdim), other=0.0
            )
            ...
            for k in range(0, dstate, BLOCK_SIZE_K):
                ...
                prev_states = tl.load(
                    prev_states_ptrs, mask=(offs_k_dstate[:, None] < dstate - k) & (offs_n[None, :] < hdim), other=0.0
                )
                prev_states = prev_states.to(C_ptr.dtype.element_ty)
    ...
    if HAS_Z:
        ...
        z = tl.load(
            z_ptrs, mask=(offs_out_m[:, None] < chunk_size_limit) & (offs_out_n[None, :] < hdim), other=0.0
        ).to(tl.float32)
        acc *= z * tl.sigmoid(z)
...
def _chunk_scan_fwd_kernel_wip(
    ...
    for start_m in range(0, chunk_size_limit, BLOCK_SIZE_M):
        ...
        dA_cs_m = tl.load(
            dA_cumsum_ptr + (start_m + offs_m) * stride_dA_cs_csize, mask=offs_m < chunk_size - start_m, other=0.0
        ).to(tl.float32)
        if HAS_SEQ_IDX:
        ...
        if not HAS_SEQ_IDX:
            ...
            scale_m = tl.where(seq_idx_m == seq_idx_prev, tl.exp(dA_cs_m), 0.0)
        C = tl.load(
            C_ptrs, mask=(offs_m[:, None] < chunk_size_limit - start_m) & (offs_k_dstate[None, :] < dstate), other=0.0
        )
        acc = tl.dot(C, prev_states.to(C_ptr.dtype.element_ty)) * scale_m[:, None]
        ...
        if start_m + BLOCK_SIZE_M < chunk_size_limit:
            ...
            B = tl.load(
                B_ptrs,
                mask=(offs_m[None, :] < chunk_size_limit - start_m) & (offs_k_dstate[:, None] < dstate),
                other=0.0,
            )
...
def _chunk_scan_bwd_dstates_kernel(
    ...
    for k in range(0, chunk_size_limit, BLOCK_SIZE_K):
        dout = tl.load(
            dout_ptrs, mask=(offs_m[:, None] < hdim) & (offs_k[None, :] < chunk_size_limit - k), other=0.0
        ).to(tl.float32)
        dA_cs_k = tl.load(dA_cumsum_ptrs, mask=offs_k < chunk_size - k, other=0.0).to(tl.float32)
        if not HAS_SEQ_IDX:
    ...
    dprev_states_ptr += (
        pid_b * stride_dprev_states_batch + pid_c * stride_dprev_states_chunk + pid_h * stride_dprev_states_head
    )
    offs_m = pid_m * BLOCK_SIZE_M + tl.arange(0, BLOCK_SIZE_M)
    offs_n = pid_n * BLOCK_SIZE_N + tl.arange(0, BLOCK_SIZE_N)
...
def _chunk_scan_bwd_dcb_kernel(
    ...
    if pid_n * BLOCK_SIZE_N >= (pid_m + 1) * BLOCK_SIZE_M:
        dcb_ptr += (
            pid_b * stride_dcb_batch + pid_c * stride_dcb_chunk + pid_g * stride_dcb_group + pid_s * stride_dcb_split
        )
...
def _chunk_scan_bwd_ddAcs_stable_kernel(
    ...
    for start_n in range(lo, hi, BLOCK_SIZE_N):
        ...
        cb = tl.load(
            cb_ptrs, mask=(offs_m[:, None] < chunk_size) & (offs_n[None, :] < chunk_size - start_n), other=0.0
        ).to(tl.float32)
        acc *= cb
        dA_cs_n = tl.load(
            dA_cumsum_ptr + start_n + offs_n * stride_dA_cs_csize, mask=offs_n < chunk_size - start_n, other=0.0
        ).to(tl.float32)
        acc *= tl.exp(dA_cs_m[:, None] - dA_cs_n[None, :])
...
def _chunk_scan_bwd_ddAcs_prev_kernel(
    ...
    dout_ptr += pid_b * stride_dout_batch + pid_c * chunk_size * stride_dout_seqlen + pid_h * stride_dout_head
    prev_states_ptr += (
        pid_b * stride_prev_states_batch + pid_c * stride_prev_states_chunk + pid_h * stride_prev_states_head
    )
    C_ptr += (
        pid_b * stride_C_batch + pid_c * chunk_size * stride_C_seqlen + (pid_h // nheads_ngroups_ratio) * stride_C_head
    )
...
def _chunk_scan_bwd_dz(x, z, out, dout, chunk_size, has_ddAcs=True, D=None, dz=None, recompute_output=False):
    ...
    if has_ddAcs:
        ddA_cumsum = paddle.empty([batch, nheads, nchunks, chunk_size], dtype=paddle.float32)
    if D is not None:
        BLOCK_SIZE_min = 32
        dD = paddle.empty(
    ...
    if recompute_output:
        outz = paddle.empty_like(x)
    dout_x = paddle.empty_like(dout)
    dD_strides = (
    ...
    _chunk_scan_bwd_dz_kernel[grid_dz](
        ...
        x.strides[3],
        D.strides[0] if D is not None else 0,
        *((outz.strides[0], outz.strides[1], outz.strides[2], outz.strides[3]) if recompute_output else (0, 0, 0, 0)),
        dz.strides[0],
        dz.strides[1],
        dz.strides[2],
        dz.strides[3],
        ...
        dD_strides[4],
        *(
            (ddA_cumsum.strides[0], ddA_cumsum.strides[2], ddA_cumsum.strides[1], ddA_cumsum.strides[3])
            if has_ddAcs
            else (0, 0, 0, 0)
        ),
        D is not None,
...
def _chunk_scan_bwd_dstates(C, dA_cumsum, dout, seq_idx=None, dtype=None):
    ...
    dprev_states = paddle.empty([batch, nchunks, nheads, headdim, dstate], dtype=dtype)
    ...
    _chunk_scan_bwd_dstates_kernel[grid_dstates](
        ...
        C.strides[2],
        C.strides[3],
        dprev_states.strides[0],
        dprev_states.strides[1],
        dprev_states.strides[2],
        dprev_states.strides[3],
        dprev_states.strides[4],
        dA_cumsum.strides[0],
        dA_cumsum.strides[2],
        dA_cumsum.strides[1],
        dA_cumsum.strides[3],
...
def _chunk_scan_bwd_dC(prev_states, dA_cumsum, dout, seq_idx=None, C=None, ngroups=1):
    ...
    assert tuple(prev_states.shape) == (batch, nchunks, nheads, headdim, dstate)
    assert tuple(dA_cumsum.shape) == (batch, nheads, nchunks, chunk_size)
    assert tuple(dout.shape) == (batch, seqlen, nheads, headdim)
    if seq_idx is not None:
        assert tuple(seq_idx.shape) == (batch, seqlen)
    if C is not None:
        assert tuple(C.shape) == (batch, seqlen, ngroups, dstate)
        C_strides = (C.strides[0], C.strides[1], C.strides[2], C.strides[3])
        ddA_cumsum_prev = paddle.empty([batch, nheads, nchunks, chunk_size], dtype=paddle.float32)
        ddA_cumsum_prev_strides = (
            ddA_cumsum_prev.strides[0],
            ddA_cumsum_prev.strides[2],
            ddA_cumsum_prev.strides[1],
            ddA_cumsum_prev.strides[3],
        )
    else:
        C_strides = (0, 0, 0, 0)
    ...
    dC = paddle.empty([batch, seqlen, nsplits, ngroups, dstate], dtype=paddle.float32)
    grid_dc = lambda META: (
    ...
    _chunk_scan_bwd_dc_kernel[grid_dc](
        ...
        dout.strides[2],
        dout.strides[3],
        prev_states.strides[0],
        prev_states.strides[1],
        prev_states.strides[2],
        prev_states.strides[3],
        prev_states.strides[4],
        *C_strides,
        dA_cumsum.strides[0],
        dA_cumsum.strides[2],
        dA_cumsum.strides[1],
...
def _chunk_scan_bwd_dcb(x, dt, dA_cumsum, dout, seq_idx=None, CB=None, ngroups=1):
    ...
    if CB is not None:
        ...
        CB_strides = (CB.strides[0], CB.strides[1], CB.strides[2], CB.strides[3], CB.strides[4])
        BLOCK_SIZE_M_min = 16
        ddA_cumsum = paddle.empty(
            [batch, nheads, nchunks, triton.cdiv(chunk_size, BLOCK_SIZE_M_min), chunk_size], dtype=paddle.float32
        )
        ddA_cumsum_strides = (
            ddA_cumsum.strides[0],
            ddA_cumsum.strides[2],
            ddA_cumsum.strides[1],
            ddA_cumsum.strides[3],
            ddA_cumsum.strides[4],
        )
    else:
    ...
    nheads_per_program = max(min(math.ceil(batch * nchunks * nheads / sm_count), nheads_ngroups_ratio), 1)
    nsplits = triton.cdiv(nheads_ngroups_ratio, nheads_per_program)
    dcb = paddle.empty([batch, nchunks, nsplits, ngroups, chunk_size, chunk_size], dtype=paddle.float32)
    ...
    if ddA_cumsum is not None:
        ...
        ddA_cumsum = ddA_cumsum[:, :, :, :n_valid_blocks].sum(axis=3)
...
def _chunk_scan_bwd_dx(cb, x, dt, dA_cumsum, dout, D=None):
    ...
    # if D is not None:
    #     BLOCK_SIZE_M_min = 32
    #     dD = paddle.empty(triton.cdiv(chunk_size, BLOCK_SIZE_M_min), batch, nchunks, nheads, headdim, device=D.device, dtype=paddle.float32)
    # else:
    #     dD = None
    ...
    _chunk_scan_bwd_dx_kernel[grid_dx](
        ...
        ddt.strides[1],
        ddt.strides[3],
        # dD.strides[1] if dD is not None else 0, dD.strides[2] if dD is not None else 0, dD.strides[3] if dD is not None else 0, dD.strides[4] if dD is not None else 0, dD.strides[0] if dD is not None else 0,
        D is not None,
        D.dim() == 2 if D is not None else True,
    )
    # if D is not None:
    ...
    #     n_valid_blocks = (chunk_size + BLOCK_SIZE_actual - 1) // BLOCK_SIZE_actual
    #     dD = dD[:n_valid_blocks].sum(axis=(0, 1, 2)).cast(dtype=D.dtype)
    return dx, ddt.cast(dtype=dt.dtype)
...
def _chunk_scan_bwd_ddAcs_unstable(x, dt, out, dout, ddt, D=None, subtract_ddtdt=True):
    ...
    if D is not None:
        assert tuple(D.shape) == (nheads, headdim) or D.shape[0] == nheads
    ddA_cumsum = paddle.empty_like(dt)
...
class ChunkScanFn(paddle.autograd.PyLayer):
    @staticmethod
    @custom_fwd
    def forward(ctx, B, C, x, dt, dA_cumsum, prev_states, D=None, z=None):
        ...
        if C.strides[-1] != 1:
            C = C.contiguous()
        if x.strides[-1] != 1 and x.strides[1] != 1:  # Either M or K dimension should be contiguous
            x = x.contiguous()
        if z is not None and z.strides[-1] != 1 and z.strides[1] != 1:  # Either M or K dimension should be contiguous
            z = z.contiguous()
        if D is not None and D.strides[-1] != 1:
            D = D.contiguous()
    ...
    def backward(ctx, dout):
        ...
        out, B, C, CB, x, dt, dA_cumsum, prev_states, D, z = ctx.saved_tensor()
        batch, seqlen, nheads, headdim = x.shape
        _, _, nchunks, chunk_size = dt.shape
        _, _, ngroups, dstate = B.shape
        ...
        # ddA_cumsum = paddle.einsum("bclhp,bclhp->bhcl", out.cast("float32"), dout.cast("float32")) - ddt * dt
        if z is not None:
            ddA_cumsum -= ddt * dt
        ...
        ddA_cumsum = ddA_cumsum.cast(dA_cumsum.dtype)
...
def chunk_scan_ref(B, C, x, dt, dA_cumsum, prev_states, D=None, z=None):
    ...
    decay = paddle.exp(dt_segment_sum)
    scores_decay = CB * rearrange(decay, "b h c l s -> b c h l s")
    causal_mask = paddle.tril(paddle.ones(chunk_size, chunk_size, dtype=bool), diagonal=0)
    scores_decay = scores_decay.masked_fill(~causal_mask, 0)
    out = paddle.einsum(
        "bchls,bhcs,bcshp->bclhp",
        scores_decay.cast(x.dtype),
        dt.cast(x.dtype),
        rearrange(x, "b (c s) h p -> b c s h p", c=nchunks),
    )
    state_decay_out = paddle.exp(rearrange(dA_cumsum, "b h c l -> b c l h 1"))
    out_prev = (
        paddle.einsum(
            "bclhn,bchpn->bclhp", rearrange(C, "b (c l) h n -> b c l h n", c=nchunks), prev_states.cast(C.dtype)
        )
        * state_decay_out
    )
    out = out + out_prev
