...
try:
    import causal_conv1d_cuda_pd as causal_conv1d_cuda

    from ..cuda.causal_conv1d import causal_conv1d_fn
except ImportError:
...
def _mamba_chunk_scan_combined_fwd(
    ...
    assert nheads % ngroups == 0
    assert tuple(B.shape) == (batch, seqlen, ngroups, dstate)
    assert tuple(x.shape) == (batch, seqlen, nheads, headdim)
    assert tuple(dt.shape) == (batch, seqlen, nheads)
    assert A.shape[0] == nheads
    assert C.shape == B.shape
    if z is not None:
        assert z.shape == x.shape
    if D is not None:
        assert D.shape == [nheads, headdim] or D.shape[0] == nheads
    if seq_idx is not None:
        assert tuple(seq_idx.shape) == (batch, seqlen)
    if B.strides[-1] != 1:
        B = B.contiguous()
    ...
    if initial_states is not None:
        assert tuple(initial_states.shape) == (batch, nheads, headdim, dstate)
    ...
    CB = _bmm_chunk_fwd(C, B, chunk_size, seq_idx=seq_idx, output_dtype=paddle.float32)
    ...
    if cu_seqlens is None:
        ...
        varlen_states = chunk_state_varlen(
            B.squeeze(0), x.squeeze(0), dt.squeeze(0), dA_cumsum.squeeze(0), cu_seqlens, states.squeeze(0)
        )
...
def _mamba_chunk_scan_combined_bwd(
    ...
    if dB is not None:
        ...
        dB_given = paddle.empty_like(B)
    if dC is not None:
        assert dC.shape == C.shape
        dC_given = dC
    else:
        dC_given = paddle.empty_like(C)
    if dz is not None:
    ...
    if ddt is not None:
        ...
        ddt_given = paddle.empty_like(dt)
    ...
    # dC = _chunk_scan_bwd_dC(states[:, :-1].cast(x.dtype), dA_cumsum, dout, seq_idx=seq_idx, ngroups=ngroups)
    dC, ddA_cumsum_prev = _chunk_scan_bwd_dC(
        states.cast(x.dtype), dA_cumsum, dout, seq_idx=seq_idx, C=C, ngroups=ngroups
    )
    ...
    ddA_prev = ddA_cumsum_prev.flip([-1]).cumsum(axis=-1).flip([-1])
    ...
    # _, dA = selective_scan_bwd(dout, x, dt, A, B, C, D=D.cast("float32"), z=z)
    # ddt_given.copy_(ddt)
...
def selective_scan_bwd(dout, x, dt, A, B, C, D=None, z=None):
    ...
    squeeze_A = A.dim() == 1
    if A.dim() == 1:
        A = repeat(A, "h -> (h p) n", p=headdim, n=dstate).cast(dtype=paddle.float32)
    else:
        A = A.cast(dtype=paddle.float32)
    ...
    if C.strides[-1] != 1:
        C = C.contiguous()
    if z is not None and z.strides[-1] != 1:
        z = z.contiguous()
    _, intermediate, *rest = selective_scan.fwd(x, dt.cast(dtype=x.dtype), A, B, C, D, z, None, False)
    if z is not None:
        out = rest[0]
    else:
        out = None
    ...
    if squeeze_dt:
        ddt = ddt.cast("float32").sum(axis=2)
    if squeeze_A:
        dA = rearrange(dA, "(h p) n -> h p n", p=headdim).sum(axis=(1, 2))
    return ddt, dA


class MambaChunkScanCombinedFn(paddle.autograd.PyLayer):
    @staticmethod
    @custom_fwd
    def forward(
        ctx,
    ...
    def backward(ctx, dout, *args):
        out, x, dt, dA_cumsum, A, B, C, D, z, dt_bias, initial_states, seq_idx = ctx.saved_tensor()
...
def mamba_chunk_scan(x, dt, A, B, C, chunk_size, D=None, z=None, dt_bias=None, dt_softplus=False):
    ...
    if seqlen % chunk_size != 0:
        dt = F.pad(dt, (0, chunk_size - seqlen % chunk_size), data_format="NLC")
    dt = rearrange(dt, "b (c l) h -> b h c l", l=chunk_size)
    dt = dt.cast("float32")  # We want high precision for this before cumsum
    ...
    dA = dt * rearrange(A, "h -> h 1 1")
    dA_cumsum = paddle.cumsum(dA, axis=-1)
...
def ssd_chunk_scan_combined_ref(x, dt, A, B, C, chunk_size, D=None, z=None, dt_bias=None, dt_softplus=False):
    ...
    if states.dtype not in [paddle.float32, paddle.float64]:
        states = states.cast(paddle.float32)
    # 2. Pass the state to all the chunks by weighted cumsum.
    ...
    states = states.cast(states_dtype)
    # 3. Compute the output for each chunk
...
def ssd_selective_scan(x, dt, A, B, C, D=None, z=None, dt_bias=None, dt_softplus=False, dt_limit=(0.0, float("inf"))):
    ...
    if dt_limit != (0.0, float("inf")):
        ...
        if dt_softplus:
            dt = F.softplus(dt)
        dt = dt.clip(min=dt_limit[0], max=dt_limit[1]).cast(x.dtype)
        dt_bias = None
        dt_softplus = None
...
def mamba_conv1d_scan_ref(
    ...
    x, B, C = paddle.split(xBC, [dim, ngroups * dstate, ngroups * dstate], axis=-1)
...
class MambaSplitConv1dScanCombinedFn(paddle.autograd.PyLayer):
    @staticmethod
    @custom_fwd
    def forward(
        ctx,
        ...
        assert d_nonssm >= 0
        assert tuple(zxbcdt.shape) == (batch, seqlen, 2 * d_nonssm + 2 * dim + 2 * ngroups * dstate + nheads)
        assert dt_bias.shape[0] == nheads
        assert A.shape[0] == nheads
        zx0, z, xBC, dt = paddle.split(zxbcdt, [2 * d_nonssm, dim, dim + ngroups * dstate * 2, nheads], axis=-1)
        ...
        x, B, C = paddle.split(xBC_conv, [dim, ngroups * dstate, ngroups * dstate], axis=-1)
        x = rearrange(x, "b l (h p) -> b l h p", h=nheads)
        B = rearrange(B, "b l (g n) -> b l g n", g=ngroups)
        C = rearrange(C, "b l (g n) -> b l g n", g=ngroups)
        z = rearrange(z, "b l (h p) -> b l h p", h=nheads) if z is not None else None
        if rmsnorm_weight is None:
            ...
            rstd = None
            if d_nonssm > 0:
                out = paddle.concat([_swiglu_fwd(zx0), out], axis=-1)
        else:
            ...
            if d_nonssm == 0:
                ...
                out01 = paddle.empty((batch, seqlen, d_nonssm + dim), dtype=x_rms.dtype)
    ...
    def backward(ctx, dout, *args):
        ...
        if recompute_output:
            out_recompute = paddle.empty([*out.shape[:2], d_nonssm + dim], dtype=out.dtype)
            out0_recompute, out1_recompute = out_recompute.split([d_nonssm, dim], axis=-1)
        zx0, z, xBC, dt = paddle.split(zxbcdt, [2 * d_nonssm, dim, dim + 2 * ctx.ngroups * dstate, nheads], axis=-1)
        # Recompute x, B, C
        xBC_conv = rearrange(
            causal_conv1d_cuda.causal_conv1d_fwd(
                rearrange(xBC, "b s d -> b d s"),
        ...
        x, B, C = paddle.split(xBC_conv, [dim, ctx.ngroups * dstate, ctx.ngroups * dstate], axis=-1)
        x = rearrange(x, "b l (h p) -> b l h p", h=nheads)
        B = rearrange(B, "b l (g n) -> b l g n", g=ctx.ngroups)
        C = rearrange(C, "b l (g n) -> b l g n", g=ctx.ngroups)
        dzxbcdt = paddle.empty_like(zxbcdt)
        dzx0, dz, dxBC_given, ddt_given = paddle.split(
            dzxbcdt, [2 * d_nonssm, dim, dim + 2 * ctx.ngroups * dstate, nheads], axis=-1
        )
        dxBC = paddle.empty_like(xBC)
        dx, dB, dC = paddle.split(dxBC, [dim, ctx.ngroups * dstate, ctx.ngroups * dstate], axis=-1)
        ...
        if d_nonssm > 0:
            dout0, dout = dout.split([d_nonssm, dim], axis=-1)
        ...
        if outproj_weight is not None:
            doutproj_weight = paddle.einsum("bso,bsd->od", dout_og, out_for_linear)
            doutproj_bias = dout_og.sum(axis=(0, 1)) if outproj_bias is not None else None
        else:
...
def mamba_split_conv1d_scan_ref(
    ...
    assert tuple(zxbcdt.shape) == (batch, seqlen, 2 * dim + 2 * ngroups * dstate + nheads)
    assert dt_bias.shape[0] == nheads
    assert A.shape[0] == nheads
    if rmsnorm_weight is not None:
        assert rmsnorm_weight.shape[0] == dim
    z, xBC, dt = paddle.split(zxbcdt, [dim, dim + 2 * ngroups * dstate, nheads], axis=-1)
    xBC = rearrange(
        causal_conv1d_fn(rearrange(xBC, "b s d -> b d s"), conv1d_weight, conv1d_bias, activation=activation),
        "b d s -> b s d",
    )
