...
def main():
    ...
    # ✅ 替换为原生 seqeval 的 compute_metrics 函数
    def compute_metrics(p):
        predictions, labels = p
        predictions = np.argmax(predictions, axis=2)

        true_predictions = [
            [label_list[p] for (p, l) in zip(pred, label) if l != -100] for pred, label in zip(predictions, labels)
        ]
        true_labels = [
            [label_list[l] for (p, l) in zip(pred, label) if l != -100] for pred, label in zip(predictions, labels)
        ]
    ...
    if training_args.do_predict:
        ...
        tokens_label = test_ret.predictions.argmax(axis=-1).tolist()
        value = []
