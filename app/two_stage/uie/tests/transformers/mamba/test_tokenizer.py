...
from paddlenlp.transformers.mamba.tokenizer import MambaTokenizer
from paddlenlp.transformers.tokenizer_utils import PretrainedTokenizer

from ...transformers.test_tokenizer_common import TokenizerTesterMixin


class MambaTokenizationTest(TokenizerTesterMixin, unittest.TestCase):

    tokenizer_class = MambaTokenizer
    test_decode_token = True

    def get_tokenizer(self, **kwargs) -> PretrainedTokenizer:
        tokenizer = MambaTokenizer.from_pretrained("state-spaces/mamba-2.8b-hf", **kwargs)
        return tokenizer
    ...
    def test_full_tokenizer(self):
        ...
        text = "lower newer"
        bpe_tokens = ["Ġlower", "Ġnewer"]
        tokens = tokenizer.tokenize(text, add_prefix_space=True)
        self.assertListEqual(tokens, bpe_tokens)

        input_tokens = tokens + [tokenizer.unk_token]
        input_bpe_tokens = [2406, 21629, 0]
